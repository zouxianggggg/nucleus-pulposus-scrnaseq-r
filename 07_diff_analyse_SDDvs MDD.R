...
# 确保 group 列中有 SDD 和 MDD 组信息
table(scRNAsub@meta.data$group)

# 找出实验组（SDD）和对照组（MDD）之间的差异基因
diff_genes <- FindMarkers(scRNAsub,ident.1 = "SDD", ident.2 = "MDD", group.by = "group",min.pct = 0.1) %>%
...
# avg_exp <- rowMeans(GetAssayData(scRNAsub, layer = "count"))
# 
# # 将平均表达量添加到差异分析结果中
...
####
#存储topleft信息
save(top_left,file = "./SDDvsMDD差异基因.Rdata")
...
ggplot(diff_genes_filter_test, aes(x = avg_log2FC, y = Delta_Percentage, color = highlight)) +
  ...
  labs(title = "Scatter Plot with Highlighted Genes",
       ...
       color = "Gene Status")
# 保存前 100 个基因到 CSV 文件
write.csv(highlight_genes, file = "差异基因SDDvsMDD.csv", row.names = FALSE)
...
# DoHeatmap(scRNAsub, features = sig_genes) + 
...
write.csv(topn_downregulated, file = "SDDvsMDD_top_100_differential_genes.csv", row.names = FALSE)
...
top_downregulated <- top_downregulated[rev(rownames(top_downregulated)), ]
# 合并上调和下调的基因列表到数据框
...
# 将基因保存为CSV文件
write.csv(top_genes_df, file = "SDDvsMDD_top_20_genes.csv", row.names = FALSE)
# 查看前 100 个基因
print(top_100_genes)
Idents(scRNAsub) <- "group"
# # 使用 Seurat 绘制前 100 个差异基因的热图
ggp = DoHeatmap(scRNAsub, features = top_genes,group.by = "group") +
  scale_fill_gradientn(colors = c("blue", "white", "red")) +
  labs(title = "Top 100 Differentially Expressed Genes Heatmap")
# 保存 ggp 对象为 PDF
ggsave("heatmap_top_genes.pdf", plot = ggp, width = 10, height = 10, units = "in")
# 保存前 100 个基因到 CSV 文件
write.csv(top_100_genes, file = "SDDvsMDD_top_100_differential_genes.csv", row.names = FALSE)
